...
//subprogram de copiere a datelor din vector pentru a se face sortarea pe exact aceleasi date
void Copiere(int aux[], int v[], int n)
{
    for(int i = 0; i < n; i ++)
        aux[i] = v[i];
}
...
{   
    ...
    for(int t = 1; t <= nt; t ++)
    {
        ...
        fout << "Testul nr." << t << ": N = " << n << " MaxN = " << nmax <<"\n";
        ...
        auto diff = end - start;
        fout << "Sort -> timp: " << chrono::duration<double, milli>(diff).count() << " ms" << "\n";
        ...
        diff = end - start;
        fout << "CountSort -> Ordonat: " << Verificare(a, v, n) << " -> timp: " << chrono::duration<double, milli>(diff).count() << " ms" << endl;
        ...
        diff = end - start;
        fout << "MergeSort -> Ordonat: " << Verificare(a, v, n) << " -> timp: " << chrono::duration<double, milli>(diff).count() << " ms" << endl;
